...
<div class="container">
    ...
    <form action="{{ route('holidays.update', $holiday->id) }}" method="POST" id="holidayForm">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <div id="adDateContainer">
                ...
                <input type="date" class="form-control" id="date" name="date" value="{{ $holiday->date }}" onchange="convertToNepali()">
            </div>
            ...
            <input type="hidden" id="hidden_date" name="date" value="{{ $holiday->date }}">
            <input type="hidden" id="hidden_nepali_date" name="nepali_date" value="{{ $holiday->nepali_date }}">
        </div>
    ...
    if (adDate) {
        ...
        .then(data => {
            if (data.nepali_date) {
                ...
                document.getElementById('nepali_date').value = data.nepali_date;
            }
        })
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If we have existing nepali date but no conversion was done, auto-convert
    const existingNepaliDate = '{{ $holiday->nepali_date }}';
    const existingAdDate = '{{ $holiday->date }}';
    
    if (existingAdDate && !existingNepaliDate) {
        convertToNepali();
    }
});
</script>
